...
export default function TerceroUpdateForm(props) {
  const {
    id: idProp,
    tercero: terceroModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = terceroRecord
      ? { ...initialValues, ...terceroRecord }
      : initialValues;
    setTipo_tercero(cleanValues.tipo_tercero);
    setIdentificacion(cleanValues.identificacion);
    setDv(cleanValues.dv);
    setExpedida(cleanValues.expedida);
    setLugar_nacimiento(cleanValues.lugar_nacimiento);
    setFecha_nacimiento(cleanValues.fecha_nacimiento);
    setNombre_completo(cleanValues.nombre_completo);
    setPrimer_apellido(cleanValues.primer_apellido);
    setSegundo_apellido(cleanValues.segundo_apellido);
    setPrimer_nombre(cleanValues.primer_nombre);
    setSegundo_nombre(cleanValues.segundo_nombre);
  ...
  const [terceroRecord, setTerceroRecord] = React.useState(terceroModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Tercero, idProp)
        : terceroModelProp;
      setTerceroRecord(record);
    };
    queryData();
  }, [idProp, terceroModelProp]);
  React.useEffect(resetStateValues, [terceroRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Tercero.copyOf(terceroRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "TerceroUpdateForm")}
      {...rest}
    >
      <SelectField
        label="Tipo tercero"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || terceroModelProp)}
