...
export default function AlmacenUpdateForm(props) {
  const {
    id: idProp,
    almacen: almacenModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = almacenRecord
      ? { ...initialValues, ...almacenRecord }
      : initialValues;
    setCodigo(cleanValues.codigo);
    setNit(cleanValues.nit);
    setNombreAlmacen(cleanValues.nombreAlmacen);
    setDireccion(cleanValues.direccion);
    setCiudad(cleanValues.ciudad);
    setTelefono(cleanValues.telefono);
    setSecciones(cleanValues.secciones ?? []);
    setCurrentSeccionesValue("");
    setEstantes(cleanValues.estantes ?? []);
    setCurrentEstantesValue("");
    setNiveles(cleanValues.niveles ?? []);
    setCurrentNivelesValue("");
    setCajas(cleanValues.cajas ?? []);
    setCurrentCajasValue("");
    setEstado(cleanValues.estado);
    setErrors({});
  };
  const [almacenRecord, setAlmacenRecord] = React.useState(almacenModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Almacen, idProp)
        : almacenModelProp;
      setAlmacenRecord(record);
    };
    queryData();
  }, [idProp, almacenModelProp]);
  React.useEffect(resetStateValues, [almacenRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Almacen.copyOf(almacenRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AlmacenUpdateForm")}
      {...rest}
    >
      <TextField
        label="Codigo"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || almacenModelProp)}
