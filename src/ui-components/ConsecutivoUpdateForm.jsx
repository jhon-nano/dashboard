...
export default function ConsecutivoUpdateForm(props) {
  const {
    id: idProp,
    consecutivo: consecutivoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = consecutivoRecord
      ? { ...initialValues, ...consecutivoRecord }
      : initialValues;
    setConsecutivo(cleanValues.consecutivo);
    setCodigo(cleanValues.codigo);
    setErrors({});
  };
  const [consecutivoRecord, setConsecutivoRecord] =
    React.useState(consecutivoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Consecutivo, idProp)
        : consecutivoModelProp;
      setConsecutivoRecord(record);
    };
    queryData();
  }, [idProp, consecutivoModelProp]);
  React.useEffect(resetStateValues, [consecutivoRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Consecutivo.copyOf(consecutivoRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ConsecutivoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Consecutivo"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || consecutivoModelProp)}
