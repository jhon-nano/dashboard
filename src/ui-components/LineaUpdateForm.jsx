...
export default function LineaUpdateForm(props) {
  const {
    id: idProp,
    linea: lineaModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = lineaRecord
      ? { ...initialValues, ...lineaRecord }
      : initialValues;
    setNombreLinea(cleanValues.nombreLinea);
    setEstado(cleanValues.estado);
    setErrors({});
  };
  const [lineaRecord, setLineaRecord] = React.useState(lineaModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Linea, idProp)
        : lineaModelProp;
      setLineaRecord(record);
    };
    queryData();
  }, [idProp, lineaModelProp]);
  React.useEffect(resetStateValues, [lineaRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Linea.copyOf(lineaRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "LineaUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nombre linea"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || lineaModelProp)}
