...
import { LazyPedido, Pedido, PedidoItem, TipoPedidos } from "../models";
...
export default class PedidoHelpers {

  private moduloPedidos: TypesPedido;
  private moduloVentas: TypesVenta;
  private confirm: any;
  ...
  constructor(store: any,confirm: any,notificacion: any,router:any,dispatch: any) {
    ...
    this.moduloVentas = new TypesVenta(router);
  ...
  onSubmitCreatePedido = (data: any) => {
   ...
   try{     
        ...
        if(data.forma_pago.value !== TipoPedidos.SEPARADO){
          ...
          cambio = await this.openPaymentDialog(data.total); // Asumiendo que `data.total` es el total del pedido
        }else{
          cambio = 0
        }
        ...
        if (pedido !== null) {
          ...
          this.moduloVentas.pushPathView(pedido.id,'/pedido');
            
        }
        }else{
          this.notificacion("Pedido Cancelado!", {
            variant: "warning",
          })
        }
    ...
    .then(async () => {
      try {
        ...
        if(cambio !== null){
          ...
          let pedidoData = this.utilsFormat.formatPedidoUpdateData({...data, cambio})
  ...
  handleEditarPedido (pedido: any) {
    ...
    .then(async () => {
      this.moduloVentas.pushPathUpdate(pedido.id,'/pedido')
    })
    .catch((error: any) => {
      console.error(error)
    });
  ...
  handleFinalizarPedidoSeparado (data: any, forma_pago: string, items: any) {
    ...
    .then(async () => {
        ...
        // Aquí abriríamos el diálogo para el pago
        let cambio = await this.openPaymentDialog(data.total); // Asumiendo que `data.total` es el total del pedido
